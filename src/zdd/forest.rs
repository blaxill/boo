...
impl Forest {
    ...
    pub fn support_count(&self, node: NodeId) -> usize {
        if node == 0 { return 0 }
        if node == 1 { return 1 }

        let l = self.follow_low(node);
        let r = self.follow_high(node);
        let l_support = self.support_count(l);
        let r_support = self.support_count(r);

        l_support + r_support
    }

    pub fn cmp(&mut self, lhs: NodeId, rhs: NodeId) -> Ordering {
        match (lhs, rhs) {
        ...
        let mut l_lead = self.lead(lhs);
        let mut r_lead = self.lead(rhs);

        if l_lead == r_lead { return Ordering::Equal }

        let l_degree = self.degree(lhs);
        let r_degree = self.degree(rhs);

        if l_degree > r_degree { return Ordering::Greater }
        if l_degree < r_degree { return Ordering::Less }

        loop {
            let l_v = self.get_variable(l_lead).unwrap();
            let r_v = self.get_variable(r_lead).unwrap();

            if l_v < r_v { return Ordering::Less }
            if l_v > r_v { return Ordering::Greater }

            l_lead = self.follow_high(l_lead);
            r_lead = self.follow_high(r_lead);
